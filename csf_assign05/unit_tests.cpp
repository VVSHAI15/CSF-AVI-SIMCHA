...
public:
  ...
  TableGuard(Table *table) : m_table(table) { m_table->lock(); }

  ~TableGuard() { m_table->unlock(); }
};
...
TestObjs::TestObjs()
    : m()
      ...
      push_req(MessageType::PUSH, {"47374"}), pop_req(MessageType::POP),
      set_req(MessageType::SET, {"accounts", "acct123"}),
      get_req(MessageType::GET, {"accounts", "acct123"}),
      add_req(MessageType::ADD), mul_req(MessageType::MUL),
      sub_req(MessageType::SUB), div_req(MessageType::DIV),
      bye_req(MessageType::BYE), ok_resp(MessageType::OK)
      ...
      data_resp(MessageType::DATA, {"10012"}), long_get_req(MessageType::GET)
      ...
      ,
      invalid_create_req(MessageType::CREATE,
                         {"8foobar"}) // arg is not an identifier
      ,
      ...
      encoded_get_req_too_long("GET foo " +
                               std::string(Message::MAX_ENCODED_LEN, 'x'))

      // Table objects
      ,
      invoices(new Table("invoices")), line_items(new Table("line_items")) {
...
TestObjs *setup() { return new TestObjs; }

void cleanup(TestObjs *objs) { delete objs; }
...
void test_message_serialization_encode_long(TestObjs *objs) {
  std::string expected_str =
      "GET " + std::string(509, 'y') + " " + std::string(509, 'y') + "\n";
