...
Cache::Cache(bool writeAllocate, bool writeThrough, bool isLRU,
  ...
  sets.resize(setsCount); // Resize the vector of sets according to the number of sets
  for (auto &set : sets) {
    set.blocks.resize(blocksPerSet);  // Resize each set's block vector to the number of blocks per set
  }
}
...
void Cache::store(uint32_t setIndex, uint32_t tag, uint32_t blockSize) {
  ...
  if (blockIter != set.index.end()) { // Store hit
    ...
    if (writeThrough) {
      ...
      set.blocks[blockIter->second].isDirty = true; // Mark block as dirty for write-back
    }
    ++saveHits;
...
void Cache::handleMiss(Set &set, uint32_t tag, uint32_t blockSize,
  ...
  for (auto it = set.blocks.begin(); it != set.blocks.end(); ++it, ++index) {
    if (!it->valid) {  // Found an invalid block
      replaceBlock(set, index, tag, blockSize, isLoad); // Replace the invalid block
      foundInvalid = true;
  ...
  if (!foundInvalid) { // No invalid block found, replace LRU or FIFO block
    ...
    replaceBlock(set, minIndex, tag, blockSize, isLoad);  // Replace the LRU/FIFO block
  }
}
