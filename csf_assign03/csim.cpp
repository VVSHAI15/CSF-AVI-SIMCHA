...
int main(int argc, char * argv[]) {
    ...
    while (getline(cin, trace)) {
        uint32_t addr = stol(trace.substr(4,8), 0, 16); //mem address from the trace
        uint32_t tag = addr >> (logBlockSize + logSets); //upper bits representing the tag
        uint32_t setIndex = addr << (32 - (logBlockSize + logSets)); //lower bits representing the index
